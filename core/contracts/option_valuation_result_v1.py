...
class OptionValuationResultV1:
    """Immutable governed valuation result contract for single-trade Phase C option valuation."""
    ...
    valuation_measures: tuple[ValuationMeasureResultV1, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.valuation_measures) != len(PHASE_C_CANONICAL_VALUATION_MEASURE_ORDER_V1):
            raise ValueError("valuation_measures must contain exactly the approved Phase C measure set")

        for measure in self.valuation_measures:
            if not isinstance(measure, ValuationMeasureResultV1):
                raise ValueError("valuation_measures entries must be ValuationMeasureResultV1")
        ...
        if provided_order != PHASE_C_CANONICAL_VALUATION_MEASURE_ORDER_V1:
            raise ValueError("valuation_measures must match approved Phase C measures in canonical order")


__all__ = ["OptionValuationResultV1"]
