...
class ButterflyConfig:
    """קונפיגורציה בסיסית ל-Long Butterfly."""
...
def _build_butterfly_ui(df_view: pd.DataFrame) -> ButterflyConfig:
    """
    UI לבחירת סטרייקים וכמות עבור Long Butterfly.
    """
    st.markdown("#### Butterfly parameters")

    strikes = sorted(df_view["strike"].unique().tolist())

    if len(strikes) < 3:
        st.warning("נדרשים לפחות שלושה סטרייקים כדי לבנות Butterfly.")
        # ערכי דיפולט ליתר ביטחון
...
def _compute_butterfly_payoff(
    cfg: ButterflyConfig, df_view: pd.DataFrame
) -> tuple[dict, pd.DataFrame]:
    """
    מחשב מטריקות ו-payoff עבור Long Butterfly (CALL/PUT) עם פרמיות מהשרשרת.
    מבנה: +1 lower, -2 middle, +1 upper.
    """
    ...
    # עלות נטו ליחידה
    # long lower + long upper - 2 * short middle
    net_debit_per_unit = p_lower + p_upper - 2.0 * p_middle
    total_debit = net_debit_per_unit * cfg.multiplier * cfg.qty
    ...
    max_profit_approx = max(
        0.0, (wing_min - net_debit_per_unit) * cfg.multiplier * cfg.qty
    )
    max_loss = -total_debit

    # Break-even בקירוב (אם wings בערך סימטריים)
    lower_be = cfg.middle_strike - wing_min + net_debit_per_unit
    upper_be = cfg.middle_strike + wing_min - net_debit_per_unit
...
def render_butterfly(df_view: pd.DataFrame) -> None:
    """
    פונקציה שמתאימה ל-StrategyDefinition.render(df_view):
    מציירת את כל ה-UI של Long Butterfly על בסיס df_view.
    """

    st.markdown("### Long Butterfly (1:-2:1)")
    ...
    cfg = _build_butterfly_ui(df_view)
    ...
    try:
        metrics, df_pay = _compute_butterfly_payoff(cfg, df_view)
    ...
    c2.metric("Max loss (approx.)", f"{metrics['max_loss']:.2f}")
    c3.metric("Max profit (approx.)", f"{metrics['max_profit_approx']:.2f}")
    ...
    st.caption(
        ...
        f"Net debit / unit={metrics['net_debit_per_unit']:.2f}"
    )

    # --- גרף payoff ---
    st.markdown("#### Payoff at expiry")

    fig = px.line(df_pay, x="S", y="P/L", title="Long Butterfly payoff (expiry)")
    fig.add_hline(y=0)
