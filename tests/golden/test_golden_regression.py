...
def test_golden_expected_matches_computed():
    expected_path = Path("tests/golden/expected/bs_smoke/expected_v1.json")
    expected = load_expected(expected_path)

    inputs = json.loads(Path(expected["units"] and "tests/golden/datasets/bs_smoke/inputs_v1.json" ).read_text())

    # For each case, compute price + greeks, canonicalize, and compare
    for exp_case, inp in zip(expected["cases"], inputs):
        id = exp_case["id"]
        # compute
        opt = inp["option_type"]
        spot = float(inp["spot"])
