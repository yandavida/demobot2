...
from core.services.pricing_artifact_builder_v2 import __name__ as builder_module_name
...
def test_successful_build_from_full_canonical_result() -> None:
    artifact = build_option_pricing_artifact_v2(valuation_result=_full_canonical_result())

    assert isinstance(artifact, OptionPricingArtifactV2)


def test_successful_build_from_model_direct_slice_result() -> None:
    artifact = build_option_pricing_artifact_v2(valuation_result=_model_direct_result())

    assert isinstance(artifact, OptionPricingArtifactV2)


def test_builder_populates_exact_artifact_identity_constants() -> None:
    artifact = build_option_pricing_artifact_v2(valuation_result=_full_canonical_result())

    assert artifact.artifact_contract_name == ARTIFACT_CONTRACT_NAME_V2
    assert artifact.artifact_contract_version == ARTIFACT_CONTRACT_VERSION_V2


def test_builder_hash_matches_direct_canonical_hash_function() -> None:
    valuation_result = _full_canonical_result()
    artifact = build_option_pricing_artifact_v2(valuation_result=valuation_result)

    expected_hash = canonical_option_pricing_artifact_payload_hash_v2(valuation_result=valuation_result)
    assert artifact.canonical_payload_hash == expected_hash


def test_repeated_builds_with_same_input_are_identical() -> None:
    valuation_result = _full_canonical_result()

    first = build_option_pricing_artifact_v2(valuation_result=valuation_result)
    second = build_option_pricing_artifact_v2(valuation_result=valuation_result)

    assert first == second


def test_builder_rejects_wrong_input_type() -> None:
    with pytest.raises(ValueError, match="OptionValuationResultV2"):
        build_option_pricing_artifact_v2(valuation_result={})  # type: ignore[arg-type]


def test_builder_does_not_mutate_source_valuation_result() -> None:
    valuation_result = _full_canonical_result()
    before = valuation_result

    artifact = build_option_pricing_artifact_v2(valuation_result=valuation_result)

    assert valuation_result == before
    assert artifact.valuation_result is valuation_result


def test_theta_lineage_is_preserved_for_full_canonical_input() -> None:
    valuation_result = _full_canonical_result()
    artifact = build_option_pricing_artifact_v2(valuation_result=valuation_result)

    assert artifact.valuation_result.theta_roll_boundary_contract_name == THETA_ROLLED_INPUT_BOUNDARY_CONTRACT_NAME_V1
    assert artifact.valuation_result.theta_roll_boundary_contract_version == THETA_ROLLED_INPUT_BOUNDARY_CONTRACT_VERSION_V1
...
def test_no_theta_lineage_is_synthesized_for_model_direct_slice() -> None:
    artifact = build_option_pricing_artifact_v2(valuation_result=_model_direct_result())

    assert artifact.valuation_result.theta_roll_boundary_contract_name is None
    assert artifact.valuation_result.theta_roll_boundary_contract_version is None
    assert artifact.valuation_result.theta_roll_boundary_reference is None


def test_builder_module_has_no_hidden_runtime_resolver_or_pricing_imports() -> None:
...
def test_builder_module_contract_surface_is_narrow() -> None:
